...
from datasets_prep.brain_datasets import CreateDatasetMultiCoil
...
def train(rank, gpu, args):
    from utils.models.discriminator import Discriminator_small, Discriminator_large
    from utils.models.ncsnpp_generator_adagn import NCSNpp
    from utils.EMA import EMA
    crop = transforms.CenterCrop((288, 384))
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        if rank == 0:
            if epoch % 10 == 0:
                x_pos_sample = torch.abs(torch.complex(x_pos_sample[:, [0], :], x_pos_sample[:, [1], :]))
                torchvision.utils.save_image( crop(x_pos_sample), os.path.join(exp_path, 'xpos_epoch_{}.png'.format(epoch)), normalize=True)
            x_t_1 = torch.randn_like(real_data)
            fake_sample = sample_from_model(pos_coeff, netG, args.num_timesteps, x_t_1, T, args)
            fake_sample = torch.abs(torch.complex(fake_sample[:, [0], :], fake_sample[:, [1], :]))
            torchvision.utils.save_image(crop(fake_sample), os.path.join(exp_path, 'sample_discrete_epoch_{}.png'.format(epoch)), normalize=True)
